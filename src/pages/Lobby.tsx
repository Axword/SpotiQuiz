...
import { peerManager, PeerMessage } from '../lib/peerManager';
...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    const initPeer = async () => {
      try {
        await peerManager.init(roomCode, isHost);
        setConnecting(false);
        ...
        if (!isHost) {
          peerManager.send({ 
            ...
            payload: { 
              name: userName, 
              id: `spotiquiz-${roomCode}-${Date.now()}` 
            } 
          });
        }
      } catch (err) {
    ...
    const handleMessage = (message: PeerMessage, _conn: DataConnection) => {
      ...
      switch (message.type) {
        case 'join':
          if (isHost) {
            ...
            setTimeout(() => {
              ...
              peerManager.broadcast({ type: 'player-list', payload: currentPlayers });
            }, 100);
          }
          break;
        ...
        case 'start-game':
          if (!isHost) {
            console.log('Client received start-game');
    ...
    peerManager.onMessage(handleMessage);
    initPeer();

    return () => {
      peerManager.offMessage(handleMessage);
    };
  }, [roomCode, isHost, userName, navigate, addPlayer, setPlayers, setGameState, startGame]);
  ...
  const handleStartGame = () => {
    ...
    console.log('Host starting game with players:', players);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#1a1a2e] via-[#16213e] to-[#0f0f23] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">{isHost ? 'Tworzenie pokoju...' : 'Łączenie z hostem...'}</p>
          {error && (
            <div className="mt-4">
