...
export default function Game() {
  ...
  const startMusic = useCallback(async () => {
    ...
    if (roomType === 'Party' && !isHost) return;
  ...
  }, [token, deviceId, currentTrack, roomType, isHost]);
  ...
  }, [token, deviceId, roomType, isHost]);
  ...
  const handleSubmitAnswer = async (selectedAnswer?: string) => {
    if (hasAnswered || !currentTrack || !roundStartTime) return;
    ...
    const playerId = players[0]?.id || 'solo';
  ...
  const handleNextRound = () => {
    if (currentRound >= roundsCount || currentRound >= tracks.length) {
      navigate('/results');
    } else {
      nextRound();
    }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="text-gray-400">
            Runda <span className="text-white font-bold">{currentRound}</span> / {Math.min(roundsCount, tracks.length)}
          </div>
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">
            {showAnswer ? 'Odpowiedź:' : 'Jaki to utwór?'}
          </h2>
          {showAnswer && (
            ...
            {gameMode === 'List' && (
              <div className="relative">
                ...
                {showList && !hasAnswered && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-[#2a2a4a] border border-white/10 rounded-xl max-h-64 overflow-y-auto z-10">
                    {filteredTracks.slice(0, 10).map((track) => (
                      ...
                      >
                        <div className="w-10 h-10 bg-white/10 rounded flex-shrink-0">
                          {track.image && (
                            <img src={track.image} alt="" className="w-full h-full object-cover rounded" />
                          )}
                        </div>
        ...
        {showAnswer && (
          <div className="text-center space-y-6 animate-fade-in">
            ...
            >
              {currentRound >= roundsCount || currentRound >= tracks.length ? 'Zobacz wyniki' : 'Następna runda'}
